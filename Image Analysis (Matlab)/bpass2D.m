 function filtered = bpass2D(image_array, lobject, lnoise, ImComp)

 %
% NAME: bpas2D
% SYNTAX: RESULT = bpass2D(image_array, lobject, lnoise, ImComp)
% PURPOSE:  Return a grayscale image with the low frequncy and high
% frequency noise removed
% INPUT PARAMETERS: 
%   IMAGE_ARRAY: A two-dimensional image array with particles in a background 
...
filtered = filtered(1+2*lobject*[1 1]:end-2*lobject*[1 1], ...
    1+2*lobject*[1 1]:end-2*lobject*[1 1]);

% apply adapthisteq to enhance contrast in grayscale image
% Helpful for featuring but not necessary for image difference parts.
