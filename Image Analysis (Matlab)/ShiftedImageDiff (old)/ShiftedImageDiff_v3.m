...
% Iterate through times. For each time 
 for tframe = 1:1:numtframes
     ...
     if dx < 0
         MeanImageDiff(tframe) = mean(mean(mean(abs(double(StackDifference(:, 1:(xsiz-dx)))))));
     elseif dx > 0
         MeanImageDiff(tframe) = mean(mean(mean(abs(double(StackDifference(:, dx:xsiz))))));
     else
         MeanImageDiff(tframe) = mean(mean(mean(abs(double(StackDifference)))));
     end
