...
path = r'C:\Eric\Xerox Data\30um gap runs\1-6-18 0.3333Hz\1.0V\p150\u_combined_o5\linked_mod.csv'
c_range = 2.1
t_frame = 1
...
Modification History:
    ...
    mod3: restrict to particles in specific range
"""
...
# select only mobile particles if wanted
#if only_mobile:
#mobile = linked.loc[linked['dr_adj_full'] > mobility_cutoff]
mobile = linked
...
ymin = 50
ymax = 82
f_mobile_ylim = f_mobile.loc[(f_mobile['y'] > ymin) & (f_mobile['y'] < ymax)]

f = f_mobile_ylim[['xum', 'yum', 'zum']]
#else:    
#    # create dataframe with just current frame
...
f_mobile_ylim['network'] = f['network']

networked_file = linked_features_file[:-4] + '_ylim_network_2p1.csv'
f_mobile_ylim.to_csv(networked_file, index = False)
...
#print(sizes[0])
percent_singles = sizes[0,1] / float(len(f_mobile_ylim))
print('singles = ' + str(sizes[0,1]) + '/' + str(len(f_mobile_ylim)))
