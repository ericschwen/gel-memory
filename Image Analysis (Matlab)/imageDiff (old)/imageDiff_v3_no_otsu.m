function imageDiff(File_path, Numtframes, Zsiz)
...
% v3: remove otsu part to just use band pass filtered image (i want to
% check it) 1-25-17
...
meanImageDiffCenterpath = [filebase '_meanImageDiffCenter_bp.csv'];
meanXYDiffpath = [filebase '_meanXYDiff_bp.csv'];

% %Comment out particle number normalization for now
% meanXYDiffNormalizedpath = [filebase '_meanXYDiff_pnumberNorm.csv'];
...
%         % Comment out "particle number normalization" for now. Could
%         % combine with otsu filtering if i wanted?
...
%         
    ...
    % timestep
    % Set center section to all at the time being. Removing edges code
    % still below
%     center_section = zini + int32(zsiz/4):1:zfin - int32(270/4);
    center_section = zini:1:zfin;
