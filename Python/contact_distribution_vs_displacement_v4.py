...
Modification History:
    ...
    v4: modify input scheme so script can choose a different contact range
        column. 3-26-19
Notes:
...
#linked_features_file = r'C:\Eric\Xerox\Python\peri\1-6-18 data\128x128x50 p500\linked_peri_mod.csv'
linked_features_file = r'C:\Eric\Xerox\Python\peri\128x128x50_static\linked_peri_mod.csv'

contacts_column = 'contacts_50nm'
...
# Full contacts description
contacts_all = eval('.'.join(['f', contacts_column, 'values']))
mean_contacts_all = np.mean(contacts_all)
...
# create strings for immobile and mobile contacts values
immob_vals = '.'.join(['immobile', contacts_column, 'values'])
mob_vals = '.'.join(['mobile', contacts_column, 'values'])

# plot scatter plot of displacements vs contacts
plt.plot(eval(mob_vals), mobile.dr_adj_full.values, 'b.')
plt.xlabel('Contacts')
...
h_im, bin_edges = np.histogram(eval(immob_vals), bins=np.arange(14), density = False)

pdf_h_im = np.array(h_im)/float(sum(h_im))

mean_contacts_im = np.mean(eval(immob_vals))

print('mean contacts immobile: ' + str(mean_contacts_im))
print('number displacements immoblie: ' + str(len(immobile)))

# histogram of contacts for mobile and immobiel particles
h_m, bin_edges = np.histogram(eval(mob_vals), bins=np.arange(14), density = False)

pdf_h_m = np.array(h_m)/float(sum(h_m))

mean_contacts_m = np.mean(eval(mob_vals))
print('mean contacts mobile: ' + str(mean_contacts_m))
print('number displacements mobile: ' + str(len(mobile)))

print('mean contacts all: ' + str(mean_contacts_all))
