...
%%%%%%%%%%%%%%%%%%%%%%%%%%%% IMPORTING %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%
folder = 'C:\Eric\Xerox Data\30um gap runs\6-3-18 data\0.8V\';
fileList = {'training'};
...
% Define fit
myfittype = fittype('a*exp(-t/tau)/t^delt+b', 'dependent', {'y'},'independent', {'t'},...
    'coefficients', {'a','tau', 'delt', 'b'});
...
% lifetime = fit_coeffs(2);
% lifetime_lb = conf_ints(1,2);
% lifetime_ub = conf_ints(2,2);
...
fit_points = fit_coeffs(1) * exp(-1/fit_coeffs(2) * tt)./tt.^fit_coeffs(3) + fit_coeffs(4);
