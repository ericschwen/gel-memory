...
linked_features_file = r'C:\Eric\Xerox\Python\peri\1-6-17 data\128x128x50 p0\linked_mod.csv'
...
plt.plot(mobile.contacts_2pt1um.values, mobile.dr_adj_full.values, 'b.')
plt.xlabel('Contacts')
...
h_im, bin_edges = np.histogram(immobile.contacts_2pt1um.values, bins=np.arange(14), density = False)

pdf_h_im = np.array(h_im)/float(sum(h_im))

mean_contacts_im = np.mean(immobile.contacts_2pt1um.values)

print('mean contacts immobile: ' + str(mean_contacts_im))
print('number displacements immoblie: ' + str(len(immobile)))

# histogram of contacts for mobile and immobiel particles
h_m, bin_edges = np.histogram(mobile.contacts_2pt1um.values, bins=np.arange(14), density = False)

pdf_h_m = np.array(h_m)/float(sum(h_m))

mean_contacts_m = np.mean(mobile.contacts_2pt1um.values)
