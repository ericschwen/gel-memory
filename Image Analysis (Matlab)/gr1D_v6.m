...
%
% Mod. History:
%   v6: modifiy to be able to take input from linked file.
%   9-15-17
...
aNumber = 2;
center=data(data(:,1) > xmin + rc/aNumber & data(:,1) < xmax-rc/aNumber & data(:,2) > ymin + rc/aNumber & data(:,2) < ymax -rc/aNumber & data(:,3) > zmin + rc/aNumber & data(:,3) < zmax -rc/aNumber,1:3);
% center = data(:, 1:3);
...
for cnt=1:length(gr) % Normalization of g_r
    ...
    norm_gr(cnt,2)=(gr(cnt,2)*2/(4/3*pi*(gr(cnt,1))^2)*dr)/(length(center)* (length(center)+1));
