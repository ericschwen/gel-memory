...
stressfit = fit(time(lim1:lim2), M4(lim1:lim2) - mean(M4(lim1:lim2)), 'sin1');
plot(fit(time(lim1:lim2), M4(lim1:lim2) - mean(M4(lim1:lim2)), 'sin1'), 'r');
...
% M4_f0 = M4 - mean(M4_0(size(M4)-5000:size(M4)));
...
%     M4_GZ(start:start+interval) = ...
%         M4(start:start+interval) - mean(M4(start:start+interval));
%     
%     last = start + interval;
...
% M4_GZ(last:lengthLoop(1)) = ...
%     M4(last:lengthLoop(1)) - mean(M4(last:lengthLoop(1)));
% 
% plot(time, M4_GZ, 'g');
...
% for start = 1:interval:(lengthLoop(1)-interval)
% %     % Need to zero the starting point to make it work.
% %     % Can use M4_GZ or zero it within this loop.
...
%         M4_GZ(start:start+interval), 'sin1');
%     
