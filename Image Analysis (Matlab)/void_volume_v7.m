function [h] = void_volume_v5(File_in, Dx, edges)
...
% v7: rewrite to just output histogram based on input edges. 6-21-17
...
h = histcounts(0, edges);

% Calculate VV for each dV and add to histogram
parfor ff = 1:length(dV_positions)
    ...
    VV = 4/3*pi*radius_void^3;
    % Ignore norms that are less than the radius (point is inside a
    % particle)
    if VV > 0:
        % Add to histogram
        temp_h = histcounts(VV, edges);
        % Maybe should add this in a different way so I'm not modifying the
        % entire histogram h in each parfor iteration. Can't figure it out
        % right not though.
        h = h + temp_h;
    end
