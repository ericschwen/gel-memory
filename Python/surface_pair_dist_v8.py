...
Mod History:
    ...
    v6: change normalization to never use negative separation value. 3-26-19
    v7: fix normalization by multiplying by dr as well. Fix buffer by applying
    xsize conversion to um.
    v8: add finding of peak of gxi. 2-15-22
...
#path = r'C:\Eric\Xerox\Python\peri\1-6-18 data\128x128x50 p400\linked_peri_mod.csv'
#path = r'E:\Gardner Data\glass 11-17-20\ts-3-stacked\linked_peri_mod.csv'
path = r'E:\Gardner Data\glass_12-7-21\ts-3-stacked\linked_peri_mod.csv'
features_file = path
...
xsize_um = xsize * 0.125
ysize_um = ysize * 0.125
zsize_um = zsize * 0.12
...
all_positions = np.transpose(np.array([df['xum'],df['yum'], df['zum']]))

# Calculate number density for normalization
positions_w_buffer = all_positions[(all_positions[:, 0] > 0 + buffer_xy_um) & (all_positions[:,0] < xsize_um - buffer_xy_um) &
                               (all_positions[:, 1] > 0 + buffer_xy_um) & (all_positions[:,1] < ysize_um - buffer_xy_um) &
                               (all_positions[:, 2] > 0 + buffer_z_um) & (all_positions[:,2] < zsize_um - buffer_z_um)]
vol_w_buffer = (((xsize_um-buffer_xy_um) - (0 + buffer_xy_um)) * 
                ((ysize_um-buffer_xy_um) - (0 + buffer_xy_um)) *
                ((zsize_um-buffer_z_um) - (0 + buffer_z_um)))

n_density = len(positions_w_buffer)/(vol_w_buffer*num_frames)
...
#    test_positions_w_buffer = positions[(positions[:, 0] > 0 + buffer_xy_um) & (positions[:,0] < xsize_um - buffer_xy_um) &
#                               (positions[:, 1] > 0 + buffer_xy_um) & (positions[:,1] < ysize_um - buffer_xy_um) &
#                               (positions[:, 2] > 0 + buffer_z_um) & (positions[:,2] < zsize_um - buffer_z_um)]
#    
#    test_n_density = len(test_positions_w_buffer)/(vol_w_buffer)
    ...
    for i in range(len(positions)):
        ...
        for j in range(len(shift_vector)):
            ...
            # normalized weights for each contribution. Use xi=0 in normalization
            # for 'negative' separation values. I think this is more correct.
            if surface_separation[j] > 0:
                normalized_gxi_contribution[j] = 1/(surface_separation[j]+radii[j]+radii[i])**2
            else:
                normalized_gxi_contribution[j] = 1/(radii[j]+radii[i])**2
        ...
        if positions[i, 0] < 0 + buffer_xy_um or positions[i,0] > xsize_um - buffer_xy_um or \
           positions[i, 1] < 0 + buffer_xy_um or positions[i,1] > ysize_um - buffer_xy_um or \
           positions[i, 2] < 0 + buffer_z_um or positions[i,2] > zsize_um - buffer_z_um:
...
norm_gxi[:,1] = norm_gxi[:,1] /(4*np.pi*dr) / (particles_counted) / n_density
...
ax2.set_xlabel(r'Surface separation ($\mu$m)', fontsize = 20)
ax2.set_ylabel(r'g($\xi$)', fontsize = 20)
fig2.show()

# Find max value and max index for gxi
index_peak = np.argmax(norm_gxi[:,1])
separation_peak = norm_gxi[index_peak, 0]
