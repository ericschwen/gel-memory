...
for tframe = 1:1:numtframes
    ...
    if dx < 0
        MeanImageDiff(tframe) = mean(mean(mean(abs(StackDifference(:, 1:(xsiz+dx),:)))));
    elseif dx > 0
        MeanImageDiff(tframe) = mean(mean(mean(abs(StackDifference(:, dx:xsiz),:))));
    else
        MeanImageDiff(tframe) = mean(mean(mean(abs(StackDifference))));
    end
