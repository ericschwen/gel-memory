...
filein = 'C:\Eric\Xerox Data\30um gap runs\0.5Hz combined gel runs 1-17-17\1.0V (2nd)\zstack_post_train_static_bpass_3D_static_160\post_train_features.csv';
...
csvwrite(fileout_dist, contactDist_total);
...
% plot(contacts_pre(:,1), contacts_pre(:,2),'r:o');
title('Contact Number Distribution');
xlabel('Contacts');
ylabel('Number');
hold off
...
figure
hold on
plot(heightSet, meanContacts, 'b:o')
title('Mean Contact Number by Height');
xlabel('Height (um)');
...
for i = 1:1:length(contactDist)
    plot(contactDist{i}(:,1), contactDist{i}(:,2)/sum(contactDist{i}(:,2)), plotStyle, 'Color', map(i*7,:));
