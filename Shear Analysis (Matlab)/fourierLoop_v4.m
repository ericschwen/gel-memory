...
for i = 1:1:steps
    ...
    data_loops = 4; % number of times stress data is looped
    padded_stressCut = [stressCut; (ones((data_loops-1)*length(stressCut),1)*mean(stressCut))];
    ...
    if i == 1
        figure;
        plot(fCut, P1Cut*data_loops, 'b:o'); % multiplied by data_loops to keep magnitude same. DONT REALLY UNDERSTAND WHY NECESSARY.
        xlim([0.05, 1.5])
        ylim([0, 0.025])
