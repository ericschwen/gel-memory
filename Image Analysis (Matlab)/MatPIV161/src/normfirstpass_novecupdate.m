function [xx,yy,datax,datay]=normfirstpass(A,B,N,ol,counter,idx,idy,maske)
...
%             end
	    ...
	    if isnan(std(C(:)))~=1 & isnan(std(D(:)))~=1 & sum(std(D(:)))~=0 & sum(std(C(:)))~=0
	      %%%%%%%%%%%%%%%%%%%%%%%Calculate the normalized correlation:   
	      R=normxcorr2(C,D);%/(N*M*stad1*stad2);
		...
		idx(cj,ci)=nan; idy(cj,ci)=nan; max_x1=nan; max_y1=nan;
	      ...
	      datax(cj,ci)=NaN; datay(cj,ci)=NaN; %SnR(cj1,ci1)=NaN; Pkh(cj1,ci1)=NaN; 
	      x(cj,ci)=(ii+(M/2)-1);
	      y(cj,ci)=(jj+(N/2)-1);
            end
