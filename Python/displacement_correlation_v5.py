...
v4: fix problem with g(r) normalization (lack thereof)
v5: calculate based only on particle in center (like for correct g(r) calculation)
...
# create a center region 
center = df[(df.xum >= df.xum.min() + rc*buffer_frac) & (df.xum <= df.xum.max() - rc*buffer_frac) &
                (df.yum >= df.yum.min() + rc*buffer_frac) & (df.yum <= df.yum.max() - rc*buffer_frac) &
                (df.zum >= df.zum.min() + rc*buffer_frac) & (df.zum <= df.zum.max() - rc*buffer_frac)]    
...
# Declare structure to store correlation
...
# declare arrays for g_r (2 methods)
g_r = np.zeros((len(distance_edges), 2))
g_r[:, 0] = distance_edges

g_r2 = np.zeros((len(distance_edges), 2))
g_r2[:, 0] = distance_edges

for frame in range(num_frames-1):
    
    # create dataframe with just current frame. Center version and full version
    f = center.loc[center['frame'] == frame]
    f_full = df.loc[df['frame'] == frame]
    
    # declare np array for positions and displacements
    pnd = np.transpose(np.array([f['xum'],f['yum'], f['zum'],f['dr_adj_full']]))
    pnd_full = np.transpose(np.array([f_full['xum'],f_full['yum'], f_full['zum'],f_full['dr_adj_full']]))
    ...
    for i in range(len(pnd)):
            ...
            shift_vector = pnd_full - np.tile(pnd[i,:], (len(pnd_full),1))
            ## try with abs --shouldn't change anything
            # = abs(pnd_full - np.tile(pnd[i,:], (len(pnd_full),1)))
            
            # fix displacements part of shift_vector
            shift_vector[:,3] = pnd_full[:,3]
            ...
            # add distances to temporary g(r)
            temp_gr, edges = np.histogram(dnd[:,0], distance_edges)
            g_r[1:,1] = g_r[1:,1] + temp_gr
            ...
            for j in range(len(dnd)):
                ...
                # Add distance to histogram for g_r2
                g_r2[dr_bins[j], 1] = g_r2[dr_bins[j], 1] + 1
...
########### Haven't really normalized completely--just scaled correctly
## off by a constant overall (to make it 1 at long range)
######### HAVE TO REWRITE FOR MULTIPLE FRAMES (len(pnd) part)
for i in range(len(g_r)):
    if g_r[i,1] != 0:
        g_r[i,1] = (g_r[i,1]/(g_r[i,0])**2)/(len(pnd)*num_frames)
    
    # Currently commenting out g_r2 -- it's just the cumulative count
#for i in range(len(g_r2)):
#    if g_r[i,1] != 0:
#        g_r2[i,1] = (g_r2[i,1]/(g_r2[i,0])**2)/(len(pnd)*num_frames)
...
# plotting cumulative correlation
plt.plot(distance_edges, corr[:,1], 'b:o')
plt.ylabel('C(r)')
plt.xlabel('Distance r ($\mu$m)')
plt.title('Cum. Displacement Correlation')
plt.xlim(0,rc)
plt.show()
...
#plt.plot(distance_edges, g_r2[:,1], 'r:o')
plt.ylabel('g(r)')
plt.xlabel('Distance r ($\mu$m)')
...
# plotting cumulative number of particles
plt.plot(distance_edges, g_r2[:,1], 'b:o')
#plt.plot(distance_edges, g_r2[:,1], 'r:o')
plt.ylabel('Pairs per bin')
plt.xlabel('Distance r ($\mu$m)')
plt.title('Pairs per bin')
plt.xlim(0,rc)
plt.show()

# plotting averaged correlation for each bin
plt.plot(distance_edges, corr_full_avg, 'b:o')
plt.ylabel('C(r)')
plt.xlabel('Distance r ($\mu$m)')
plt.title('Normalized Displacement Correlation')
plt.xlim(0,rc)
plt.show()
