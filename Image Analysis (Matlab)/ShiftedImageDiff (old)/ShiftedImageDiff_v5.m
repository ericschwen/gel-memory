function [MeanImageDiff] = ShiftedImageDiff(file_path)
...
A2_shifted = zeros(ysiz, xsiz, zsiz);
...
for tframe = 1:1:numtframes
    ...
    for zframe = zini:dz:zsiz
        dx = round(shiftX(tframe,zframe));
        if dx < 0
            MeanXYDiff(zframe) = mean(mean(abs(StackDifference(:, (1-dx):xsiz, zframe))));
        elseif dx > 0
            MeanXYDiff(zframe) = mean(mean(abs(StackDifference(:, 1:(xsiz-dx), zframe))));
        else
            MeanXYDiff(zframe) = mean(mean(abs(StackDifference(:,:,zframe))));
        end
    end
    MeanImageDiff(tframe) = mean(MeanXYDiff);
