...
path = r'C:\Eric\Xerox\Python\peri\1-6-18 data\128x128x50 p0\linked_mod.csv'
features_file = path
...
#        surface_separation[i] = 1000
        ...
        temp_gxi, bins_x_space = np.histogram(surface_separation, bins=x_space)
...
# normalization of gxi
# NOTE: NOT WORKING YET! Cant just use old spherical boundaries from 
norm_gxi = np.array(gxi)
# NOTE: could just count particles rather than using len(positions)*num_frames
for k in range(len(gxi)):
#    norm_gxi[k,1] = gxi[k,1] / (gxi[k,1]**2) / len(positions)
    norm_gxi[k,1] = gxi[k,1] / ((gxi[k,0] + 2*np.mean(radii))**2) / (len(positions)*num_frames)
