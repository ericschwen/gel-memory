...
%    disp(['* Pass No: ',num2str(i)])
    if i==1 & ~isempty(datax)
        [x,y,datax,datay]=firstpass_gpu(A,B,wins(i,:),overlap,counter,[],[],maske);
    else
        [x,y,datax,datay]=firstpass_gpu(A,B,wins(i,:),overlap,counter,datax,datay,maske);
    end
...
%disp('* Final Pass')
[x,y,u,v,SnR,Pkh]=finalpass_gpu(A,B,wins(end,:),overlap,round(datax),...
