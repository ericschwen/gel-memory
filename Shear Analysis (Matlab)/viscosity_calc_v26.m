...
% data.
% v26: get phase information from fourier transform

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% IMPORT AND CUT DATA

filepath = ['C:\Users\Eric\Documents\Xerox Data\Shear Data\0.2Hz combined trials 1-31-17\0.6V\training\'...
    'a1_0.6000_a2_0.0000_f1_0.2000_f2_1.0000_p_00.txt'];
...
timeInterval = 240; % Time interval in seconds
...
%     timeloop(count) = 1/(freq*spp) * start;
    ...
    % PHASE calculation
    phsCut = unwrap(angle(YCut));
    lyCut = length(YCut);
    f_phaseCut = (0:lyCut-1) * (Fs/lyCut);
    f_phaseCut = f_phaseCut.';

    figure;
    hold on
    plot(f_phaseCut, phsCut/pi)
    xlabel('f (Hz)')
    ylabel('Phase / \pi')
    grid
    xlim([0.05, 0.3])
...
% some of it still works, but i'm trying the fourier part
