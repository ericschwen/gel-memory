function [params, all_paths] = lsm_parallel_import(folder)
%% Modification History
% v2: remove full_image importing to just get parameters and paths
% v3: change the shape of all_paths_shaped so that all_paths is still just
% a single column (easier to work with for later parallelization stuff).
% v4: include all_paths_shaped as function output
...
scaling = xml_metadata.ImageMetadata.Scaling.Items;
sizing = xml_metadata.ImageMetadata.Information.Image;

% Pixel size in microns
umppx.X = str2double(scaling.Distance{1}.Value.Text)*1e6;
umppx.Y = str2double(scaling.Distance{2}.Value.Text)*1e6;
% umppx.Z = str2double(scaling.Distance{3}.Value.Text)*1e6;
...
% nS = stack_size.S;
nT = stack_size.T;
out_size = [nY nX nT]; % out_size = [nY nX nC nZ nS nT];
...
for ff = 1:nPlanes
    % no longer need sub2ind since only one iterating dimension.
    ix(ff) = Tix(ff);
%     ix(ff) = sub2ind(out_supersize, Zix(ff), Tix(ff));
...
% can now call: all_paths_shaped{z,t} to get a file path.
