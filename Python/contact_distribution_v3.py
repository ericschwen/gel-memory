...
Modification History:
    ...
    effects. 3-19-19
    v3: add box plot percentile numbers
...
features_file = r'C:\Eric\Xerox\Python\peri\1-6-18 data\128x128x50 p500\linked_peri_mod.csv'
...
contacts = f.contacts_50nm.values

h, bin_edges = np.histogram(contacts, bins=np.arange(12), density = False)

pdf_h = np.array(h)/float(sum(h))

# sorting (for box plot)
contacts_sorted = np.sort(contacts)
#med = contacts_sorted[int(len(contacts_sorted)*0.5)]
#first_quart = contacts_sorted[int(len(contacts_sorted)*0.25)]
#third_quart = contacts_sorted[int(len(contacts_sorted)*0.75)]
...
#print('first_quartile ' + str(first_quart))
#print('median ' + str(med))
#print('third_quartile ' + str(third_quart))
