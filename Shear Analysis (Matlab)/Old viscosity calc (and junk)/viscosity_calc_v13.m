...
% % % silica suspension data 6-21-16
% filename = ['C:\Users\Eric\Documents\Xerox Data\Shear Data\'...
%     '0.5Hz 1V 6-21-16/a1_1.0000_a2_0.0000_f1_0.5000_f2_1.0000_p_00.txt'];

% More dense silica suspension data 6-27-16
% filename = ['C:\Users\Eric\Documents\Xerox Data\Shear Data\'...
%     '.5Hz .5V raised 6-27-16\a1_0.5000_a2_0.0000_f1_0.5000_f2_1.0000_p_00.txt'];

% % % Silica suspension 7-1-16
filename = ['C:\Eric\Xerox Data\Shear Data\'...
    '.5Hz .8V 7-1-16\a1_0.8000_a2_0.0000_f1_0.5000_f2_1.0000_p_00.txt'];
...
% Shouldn't start until time 16s or so when I zeroed the amplifiers.
% Time 16s * freq * spp = 8000 (cut index)
...
for start = 1:interval:(size(M2_0) - interval)
    Vloop = 2 * abs(fft(M2_0(start:start+interval))) / interval;
    floop = Fs * linspace(0,1,interval);
    
    [amploop(count), indexloop(count)] = max(Vloop(10:500));
...
% M2_f0 = M2 - mean(M2_0(size(M2)-5000:size(M2)));
% fit4_exp1 = fit(time(1:size(time)), M2_f0(1:size(M2_f0)), 'exp1');
% 
...
% %(NOT THE DRIFT PART)
% % Use M2_0 instead of M2_e1 if not using the version after the exponential
% % fit is subtracted.
% 
% % fits for M1 (x strain) and M2 (x stress)
% fit1 = fit(time, M1_0, 'sin1');
% fit2 = fit(time, M2_e1, 'sin1');
% 
% % Coefficient vales
...
% fit2_short = fit(time(1:len0), M2_e1(1:len0), 'sin1');
% 
% 
% plot(time(1:len0),M2_e1(1:len0), 'b');
% hold on
% plot(fit2_short,'r');
% hold off
% 
...
% len1 = 10000;
% start1 = 500000;
% M2_late = M2_e1 - mean(M2_e1(start1:start1+len1));
% fit1_late = fit(time(start1:start1+len1), M1_0(start1:start1+len1), 'sin1');
% fit2_late = fit(time(start1:start1+len1), M2_late(start1:start1+len1), 'sin1');
% 
% plot(time(start1:start1+len1),M2_late(start1:start1+len1),'b');
% hold on
% plot(fit1_late, 'g');
% plot(fit2_late, 'r');
% hold off
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % CURVE FITTING LOOP (USING EXPONENTIAL ZERO)
% % CURVE FITTING LOOP
% 
% % Use to fit curve at many points throughout the data.
% % Gives amplitude and phase angle over time.
...
%         M1_0(start:start+interval), 'sin1');
%     fitStress = fit(time(start:start+interval),...
%         M2_LZ(start:start+interval), 'sin1');
%     
%     coStrain = coeffvalues(fitStrain);
%     coStress = coeffvalues(fitStress);
%     
