...
plot(time(lim1:lim2), 0.05* M1_0(lim1:lim2),'b');
...
[ampMaxS, indexMaxS] = max(P1(index1:index2));

freqMaxS = f(indexMaxS + index1);

% get phase!
% index = frequency * L / Fs
phaseAngle1 = angle(Y(uint32(freq*L/Fs)));
phaseAngleOverPi1 = phaseAngle1/pi;

% Note: amplitude will change if you look at specific parts. Changing
% viscosity?

%% Fourier transform of piezo input signal
...
index1 = uint32(f1 * L / Fs); % get index in frequency vector
index2 = uint32(f2 * L / Fs);
[ampMaxP, indexMaxP] = max(P1(index1:index2));

freqMaxP = f(indexMaxP + index1);

% get phase!
% index = frequency * L / Fs
phaseAngle2 = angle(Y(uint32(freq*L/Fs)));
phaseAngleOverPi2 = phaseAngle2/pi;

%% Phase Difference
phaseDiff = phaseAngle2-phaseAngle1;
...
estPiezo = ampMaxP * sin(2*pi*freq*t - phaseAngle2);
estSignal = ampMaxS * cos(2*pi*freq*t - phaseAngle1);

t1 = 0;
t2 = 4;
lim1 = uint32(t1*freq*spp+1);
lim2 = uint32(t2*freq*spp+1);
figure;
...
figure;
plot(timeloop*freq, ampMaxCut, 'o');
...
%         fitStress
        ...
        hold on
        plot(time(start:start+interval), 0.05* M1_0(start:start+interval), 'b');
        t1 = (start * 1/(freq*spp));
        xlim([t1, t1+5])
        plot(fitStress);
        title('Stress and Strain vs Time')
