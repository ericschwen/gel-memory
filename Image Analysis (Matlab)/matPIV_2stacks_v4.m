...
% v4: add window options to file inputs and savepath.

function [dx_mean, dy_mean] = matPIV_2stacks_v4(File_path_1, File_path_2, zsiz, Winx, Winy)
...
strainpathX = join([filebase_1, '_v_fieldX1_', string(winx), '_', string(winy), '.csv'],'');
strainpathY = join([filebase_1, '_v_fieldY1_', string(winx), '_', string(winy), '.csv'],'');
...
%     K = imread(file2, 2*(zframe)-1 );
    ...
    windows = [winx winy; winx winy];
%     windows = [512 32; 512 32];
%     windows = [256 64; 256 64];
