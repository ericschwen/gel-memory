...
for i=1:iter-1
    ...
    if i==1 & ~isempty(datax)
        [x,y,datax,datay]=firstpass_novecupdate(A,B,wins(i,:),overlap,counter,[],[],maske);
    else
        [x,y,datax,datay]=firstpass_novecupdate(A,B,wins(i,:),overlap,counter,datax,datay,maske);
    end
...
disp('* Final Pass')
[x,y,u,v,SnR,Pkh]=finalpass_novecupdate(A,B,wins(end,:),overlap,round(datax),...
