...
elseif ~isempty(findstr(method,'multi')) %~=0
    ...
    if strcmp(method,'multin') | size(winsize,1)>1
      [x,y,u,v,SnR,Pkh]=multipassx_novecupdateorfilt(im1,im2,winsize,...
				   Dt,overlap,validvec,ms,iter, ...
				   ustart,vstart); 
    elseif strcmp(method,'multi') & size(winsize,1)==1
      [x,y,u,v,SnR,Pkh]=multipass_novecupdateorfilt(im1,im2,winsize,...
				   Dt,overlap,validvec,ms); 
    end
    
elseif strcmp(method,'mqd')==1
    [x,y,u,v,SnR,Pkh]=mqd_novecupdate(im1,im2,winsize,Dt,overlap,ms); 
elseif strcmp(method,'auto')==1
    ...
    [x,y,u,v,SnR,Pkh]=autopass_novecupdate(im1,winsize,Dt,overlap,wshift,ms);
