...
writeFileName = [filePath(1:length(filePath)-4), '_enhanced.tif'];
InfoImage=imfinfo(filePath);
mImage=InfoImage(1).Width;
nImage=InfoImage(1).Height;
numImgs=length(InfoImage)/2;

imarray = zeros(mImage,nImage,numImgs);
for i = 1:1:numImgs
    imarray(:,:,i) = imread(filePath,'Index',2*i-1,'Info',InfoImage);
end

% average brightness of total image
aveBright = mean(mean(mean(imarray)));
