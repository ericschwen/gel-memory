...
% note: need the +1 because particle numbering starts from zero
dr_adj_full_valid = zeros(length(particle_stats), 1);
for i = 1:length(particle_stats)
    dr_adj_full_valid(i) = data.dr_adj_full(particle_stats(i,1)+1);
end
particle_stats_dr = [particle_stats, dr_adj_full_valid];
