...
linked_features_file = (r'D:\Xerox Data\30um gap runs\8-29-17 0.3333Hz\0.6\p50\u_combined_o5\linked15_mod.csv')
...
plt.plot(mobile.contacts.values, mobile.dr_adj_full.values, 'b.')
plt.xlabel('Contacts')
...
h_im, bin_edges = np.histogram(immobile.contacts.values, bins=np.arange(14), density = False)

pdf_h_im = np.array(h_im)/float(sum(h_im))

mean_contacts_im = np.mean(immobile.contacts.values)

print('mean contacts immobile: ' + str(mean_contacts_im))
print('number displacements immoblie: ' + str(len(immobile)))

# histogram of contacts for mobile and immobiel particles
h_m, bin_edges = np.histogram(mobile.contacts.values, bins=np.arange(14), density = False)

pdf_h_m = np.array(h_m)/float(sum(h_m))

mean_contacts_m = np.mean(mobile.contacts.values)
