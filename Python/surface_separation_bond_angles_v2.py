...
v2: Added buffer at edges. 9-25-19.
...
save_filepath = '\\'.join(path.split('\\')[:-1]) + r'\bond_dist_50nm.csv'
...
# Set boundaries to record contacts in um
# Effectively establish a buffer. particles outside will be counted as neighbors,
# but will will not be counted for which neighbors They have.
xy_buff = 1.5
z_buff = 0.5
xmin_rec = xy_buff
xmax_rec = 16.25 - xy_buff
ymin_rec = xy_buff
ymax_rec = 16.25 - xy_buff
zmin_rec = z_buff
zmax_rec = 6.0 - z_buff
        ...
        for j in range(len(shift_vector)):
            ...
            if c2c_distance != 0:
                ...
                if (surf_sep < contact_range_um):
                    ...
                    if (pos_rad[i,0] > xmin_rec) & (pos_rad[i,0] < xmax_rec) &\
                    (pos_rad[i,1] > ymin_rec) & (pos_rad[i,1] < ymax_rec) &\
                    (pos_rad[i,2] > zmin_rec) & (pos_rad[i,2] < zmax_rec):
...
#h_a, bin_edges_a = np.histogram(azis, bins=np.arange(-180, 181, 24), density = False)
