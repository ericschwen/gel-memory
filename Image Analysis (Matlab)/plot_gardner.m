...
% Individual data points. Volume fraction then Delta
% unique first neighbors (ufn) > 1
delta_ab_ufn = [
    .585, .082; % 12-7-21 ts-3-4 relative positions
    .605, .036;  % 12-7-21 ts-7-8-9-10 relative positions
    .624, .018; % 12-7-21 ts-17-18-19 relative positions
    .632, .012; % 12-7-21 ts-27-28-29 relative positions
    ];

% delta at 90s
% unique first neighbors (ufn) > 1
delta_90_ufn = [
    .585, .041; % 12-7-21 ts-3 and ts-4 relative positions
    .605, .023; % 12-7-21 ts-8 and ts-9 and ts-10 relative positions
    .624, .0095; % 12-7-21 ts-17-18-19 relative positions
    .632, .0040; % 12-7-21 ts-27-28-29 relative positions
    ];
...
% testing mfn_sep with fewer included
mfn_sep_test = [
    .581, 6.52; % 12-7-21 ts-3 frames 1-240
    .588, 6.44; % 12-7-21 ts-4 frames 1-240
    .605, 5.99; % 12-7-21 ts-8 frames 1-240
...
% alternate calculateion of mean overlap fraction
mof_alt_calc = mfn_sep_test(:,2)./mfn_jolt_sep(:,2);

% What if i take mfn_jolt over mfn_sep? Relative cage change
rfn = mfn_jolt_sep(:,2)./mfn_sep_test(:,2);

% Mean overlap fraction of first neighbors after jolt (limits:
% 1<neighbors<16). Should probably combine and average some.
% surface separation
mof_jolt = [
    .585, 0.50; % 12-7-21 ts-3-4 frames 1-240 each
    .593, 0.67; % 12-7-21 ts-4-5 frames 1-240 each
    .605, 0.70; % 12-7-21 ts-7-8 frames 1-240 each
    .606, 0.66; % 12-7-21 ts-8-9 frames 1-240 each
    .624, 0.67; % 12-7-21 ts-17-18 frames 1-360 each
    .625, 0.81; % 12-7-21 ts-18-19 frames 1-360 each
    .632, 0.78; % 12-7-21 ts-27-28 frames 1-360 each
    .633, 0.77; % 12-7-21 ts-28-29 frames 1-360 each
    ];
...
% lines(1)=plot(delta_ab_ufn(:,1), delta_ab_ufn(:,2), 's', 'Color', blu, 'MarkerFaceColor', blu, 'MarkerSize', 8, 'Displayname', '\Delta_{AB} ufn');
% 
% % DELTA (MSD)
...
% lines(2)=plot(delta_90_ufn(:,1), delta_90_ufn(:,2), 's', 'Color', red, 'MarkerFaceColor', red, 'MarkerSize', 8, 'Displayname', '\Delta ufn');
% 
% lines(3)=plot(delta_ab(:,1), delta_ab(:,2), 'd', 'Color', blu, 'MarkerFaceColor', blu, 'MarkerSize', 8, 'Displayname', '\Delta_{AB}');
% lines(4)=plot(delta_90(:,1), delta_90(:,2), 'd', 'Color', red, 'MarkerFaceColor', red, 'MarkerSize', 8, 'Displayname', '\Delta');
% 
...
% % 
% leg = legend(lines, 'Location', 'southwest');
...
% lines = zeros(1,2);
% % mfn_jolt
% lines(1)=plot(mfn_jolt_sep(:,1), mfn_jolt_sep(:,2), 's', 'Color', blu, 'MarkerFaceColor', blu, 'MarkerSize', 8, 'Displayname', 'mfn jolt');
% % mfn
% lines(2)=plot(mfn_sep(:,1), mfn_sep(:,2), 's', 'Color', red, 'MarkerFaceColor', red, 'MarkerSize', 8, 'Displayname', 'mfn');
% leg = legend(lines, 'Location', 'northeast');



% plot(mof_jolt(:,1), mof_jolt(:,2), 's', 'Color', blu, 'MarkerFaceColor', blu, 'MarkerSize', 8, 'Displayname', 'mfn jolt');
% plot(mfn_jolt_sep(:,1),mof_alt_calc, 's', 'Color', blu, 'MarkerFaceColor', blu, 'MarkerSize', 8, 'Displayname', 'mfn jolt');
% plot(mfn_jolt_sep(:,1),rfn, 's', 'Color', blu, 'MarkerFaceColor', blu, 'MarkerSize', 8, 'Displayname', 'mfn jolt');
plot(jaccard_mean(:,1),jaccard_mean(:,2), 's', 'Color', blu, 'MarkerFaceColor', blu, 'MarkerSize', 8, 'Displayname', 'mfn jolt');
